...
execute @e[type=area_effect_cloud,name=Main,tag=!GameInProgress] ~ ~ ~ execute @a[tag=Playing] ~ ~ ~ clear @s minecraft:stone_sword -1 0
execute @e[type=area_effect_cloud,name=Main,tag=!GameInProgress] ~ ~ ~ clear @a[tag=Playing,score_AffectedItems_min=1] minecraft:stone_sword

# Determine what items need to be refilled
scoreboard players tag @a[tag=Playing] add RefillItems
execute @e[type=area_effect_cloud,name=Main,tag=GameInProgress] ~ ~ ~ execute @a[tag=RefillItems] ~ ~ ~ clear @s minecraft:stone_sword -1 0
execute @e[type=area_effect_cloud,name=Main,tag=GameInProgress] ~ ~ ~ scoreboard players tag @a[tag=RefillItems,score_AffectedItems=0] add RefillSword
...
# Diamond Pickaxe
give @a[tag=RefillSword] minecraft:stone_sword 1 0 {CanDestroy:["minecraft:stained_glass"],HideFlags:4,Unbreakable:1}
scoreboard players tag @a[tag=RefillSword] remove RefillSword
